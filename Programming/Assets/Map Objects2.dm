			...
			if(istype(A,/Player))
				...
				if(!slot)
					...
					for(connect in world)
						var/Connectors/a = connect
						if(a.slot==src.slot)
							M.loc = locate(connect)
						return 1
	Sand_Academy
		Entrance
			connect = /Connectors/Sand_Academy/Exit
			interiors=1
			Music = 'Sand Academy.ogg'
		Exit
			dir_need=SOUTH
			connect = /Connectors/Sand_Academy/Entrance
			interiors=0
			Music = 'Sand Village.ogg'
		TopFloor
			dir_need=SOUTH
			connect = /Connectors/Sand_Academy/BottomFloor
			interiors=1
		BottomFloor
			connect = /Connectors/Sand_Academy/TopFloor
			interiors=1
	Sand_Weap
		Entrance
			connect = /Connectors/Sand_Weap/Exit
			interiors=1
		Exit
			dir_need=SOUTH
			connect = /Connectors/Sand_Weap/Entrance
			interiors=0
Items
	parent_type = /obj
	Cross(mob/A)
